...
<script>
    $(document).ready(function () {
        var dataKeluarga = $('#table_keluarga').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: "{{ route('verifikasiKeluarga.list') }}",
                dataType: "json",
                type: "POST",
                data: {
                    "_token": "{{ csrf_token() }}"
        ...
        window.showDetail = function (id_keluarga) {
            window.location.href = "{{ route('verifikasiKeluarga.show', '') }}/" + id_keluarga;
        }

        window.approve = function (id_keluarga) {
            $.ajax({
                url: "{{ route('verifikasiKeluarga.approve', '') }}/" + id_keluarga,
                type: 'POST',
                data: {
                    "_token": "{{ csrf_token() }}"
                },
        ...
        window.reject = function (id_keluarga) {
            $.ajax({
                url: "{{ route('verifikasiKeluarga.reject', '') }}/" + id_keluarga,
                type: 'POST',
                data: {
                    "_token": "{{ csrf_token() }}"
                },
