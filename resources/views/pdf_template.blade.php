            ...
            <form id="suratForm">
                ...
                <table>
                    ...
                    <tr>
                        <td>
                            <label style="font-weight: normal;">
                                <input type="checkbox" id="bepergian" name="surat" value="Surat Keterangan Bepergian ke"> Surat Keterangan Bepergian ke <input type="text" name="bepergian_ke"> selama <input type="text" name="bepergian_selama">
                            </label>
                        </td>
                    </tr>
                    <tr>
                        <td>
                            <label style="font-weight: normal;">
                                <input type="checkbox" id="lainnya" name="surat" value="Surat Keterangan"> Surat Keterangan <input type="text" name="lainnya">
                            </label>
                        </td>
                    </tr>
                </table>
                <button type="button" onclick="confirmAndDownload()">Submit</button>
            </form>
...
<script>
    function confirmAndDownload() {
        // Cek apakah ada kotak centang yang dipilih
        const checkboxes = document.querySelectorAll('input[name="surat"]:checked');
        if (checkboxes.length === 0) {
            alert("Silakan pilih setidaknya satu surat.");
            return;
        }

        // Tampilkan notifikasi konfirmasi
        const confirmed = confirm("Apakah Anda yakin ingin mencetak surat?");
        if (confirmed) {
            // Pengguna mengkonfirmasi, proses pengunduhan PDF
            downloadPDF();
        }
    }
    ...
    function downloadPDF() {
        ...
        // Misalkan kita menggunakan fetch API untuk mengirim data ke server dan menerima PDF
        fetch('/generate-pdf', {
            method: 'POST',
            body: formData
        })
        ...
        .then(blob => {
            ...
            a.href = url;
            a.download = 'surat.pdf'; // Nama file yang akan diunduh
            document.body.appendChild(a); // Menambahkan elemen <a> ke dokumen
            a.click(); // Klik elemen untuk memulai unduhan
            a.remove(); // Hapus elemen setelah selesai
        })
